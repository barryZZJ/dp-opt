...
from dpopt.optimizer.optimizers import NelderMead, DualAnnealing, DifferentialEvolution, COBYLA, Powell, BruteForce
...
def run_exp_default(series_name: str, output_path: str, config: 'DPConfig'):
    runner = ExperimentRunner(output_path, series_name, log_debug=False)

    def append_exp(mechanism, input_generator: PatternGenerator, optimizer_generator: OptimizerGenerator, output_size, feature_transform=None):
        ...
        runner.experiments.append(MyExperiment(class_name(mechanism), mechanism, input_generator, factory, optimizer_generator, config))

    t0 = 0.5
    q0 = 0.5
    univariate_optimizers = [
        NelderMead(1, t0), COBYLA(1, t0), BruteForce(1), DualAnnealing(1)
    ]
    bivariate_optimizers = [
        NelderMead(2, t0, q0), DualAnnealing(2), DifferentialEvolution(2),
        COBYLA(2, t0, q0), Powell(2, t0, q0), BruteForce(2)
    ]
